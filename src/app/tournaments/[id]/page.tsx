...
export default function TournamentDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Check ticket ownership through linked event
  ...
  const handleBuyTicket = async () => {
    ...
    try {
      ...
      // Redirect to a Stripe checkout flow
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="bg-white/5 rounded-xl p-4">
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="text-lg font-semibold capitalize">{tournament.status.toLowerCase()}</p>
              </div>
              <div>
                <span className="text-gray-400">Ticket Price</span>
                <p className="text-lg font-semibold">${tournament.ticketPrice}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {showMpesaOptions && mpesaData && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl max-w-md w-full p-6 space-y-5">
            ...
            <div className="bg-black/50 rounded-xl p-4 space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-bold text-lg text-purple-400 break-all text-right">
                  {mpesaData.accountRef}
                </span>
              </div>
            </div>

            <div className="bg-yellow-900/30 border border-yellow-600/30 rounded-xl p-4 text-sm text-yellow-200">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 border border-white/20 rounded-lg py-2 hover:bg-white/10 transition-colors"
              >
                Close
              </button>
              <button
                ...
                className="flex-1 bg-purple-600 rounded-lg py-2 hover:bg-purple-700 transition-colors"
              >
                I&apos;ve Sent
              </button>
            </div>
